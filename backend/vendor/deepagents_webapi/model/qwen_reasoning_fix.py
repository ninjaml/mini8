...
Qwen3/3.5 系列通过 enable_thinking 参数开启 thinking，返回 reasoning_content 字段。
...
3. 自动在请求中添加 enable_thinking: True
"""
...
class ChatQwenWithReasoning(ChatOpenAI):
    """增强的 ChatOpenAI，支持 Qwen 的 thinking 模式。

    通过 model_kwargs.extra_body 传递 enable_thinking 参数，
    避免 OpenAI SDK 参数校验报错。
    """

    def __init__(self, **kwargs):
        # extra_body 直接传给 ChatOpenAI（它是显式参数），避免 model_kwargs 警告
    ...
    def _get_request_payload(
        ...
        **kwargs: Any,
        ...
        """注入 assistant 消息的 reasoning_content。"""
