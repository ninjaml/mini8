...
def _validate_path(path: str, *, allowed_prefixes: Sequence[str] | None = None) -> tuple[str, bool]:
    r"""Validate and normalize file path for security on Windows.

    Ensures paths are safe to use by preventing directory traversal attacks
    and enforcing consistent formatting for Windows. Detects Git Bash virtual paths
    and returns a flag indicating their use.
    ...
    Returns:
        Tuple of (normalized Windows path, was_virtual_path) where was_virtual_path
        is True if the input was a Git Bash virtual path format (which should be avoided).

    Raises:
        ValueError: If path contains traversal sequences (`..` or `~`), or does not start with an
            allowed prefix when `allowed_prefixes` is specified.

    Example:
        ```python
        # Git Bash virtual paths (detected but should be avoided)
        validate_path("/c/Users/Username/file.txt")  # Returns: ("C:/Users/Username/file.txt", True)
        validate_path("/d/Projects/code.py")  # Returns: ("D:/Projects/code.py", True)
        
        # Windows native paths (forward slashes) - preferred format
        validate_path("C:/Users/Username/file.txt")  # Returns: ("C:/Users/Username/file.txt", False)
        validate_path("D:/Projects/code.py")  # Returns: ("D:/Projects/code.py", False)
        
        # Rejected paths
        validate_path("../etc/passwd")  # Raises ValueError
        validate_path("~/secret.txt")  # Raises ValueError
        ```
    """
    if ".." in path or path.startswith("~"):
        msg = f"Path must use Windows native format (e.g., C:/Users/name/file.txt). Path traversal sequences are not allowed: {path}"
    ...
    was_virtual_path = False
    
    # Check for Git Bash virtual path format: /drive_letter/... incase it is provided
    virtual_path_match = re.match(r"^/([a-zA-Z])/(.*)", path)
    if virtual_path_match:
        drive_letter, rest = virtual_path_match.groups()
        # Convert to Windows path format
        windowspath = f"{drive_letter.upper()}:/{rest}"
        path = windowspath
        was_virtual_path = True
    
    # Ensure path is absolute (starts with drive letter)
    if not re.match(r"^[A-Za-z]:/", path):
        msg = f"Path must be absolute Windows path (e.g., C:/Users/name/file.txt): {path}"
        raise ValueError(msg)
    
    # Check allowed prefixes if specified
    if allowed_prefixes is not None and not any(path.startswith(prefix) for prefix in allowed_prefixes):
        msg = f"Path must start with one of {allowed_prefixes}: {path}"
        raise ValueError(msg)
    
    return path, was_virtual_path
...
def _is_absolute_glob_pattern(pattern: str) -> bool:
    normalized = pattern.replace("\\", "/")
    is_absolute = bool(re.match(r"^[A-Za-z]:/", normalized) or normalized.startswith("/"))
    if is_absolute:
        print(f"[glob middleware] absolute glob pattern detected: {normalized}")
    return is_absolute


LIST_FILES_TOOL_DESCRIPTION = r"""Lists all files in the filesystem, filtering by directory.

Usage:
- The path parameter must be an absolute Windows path
  - Windows native format: `C:/Users/name/Documents` (forward slashes)
...
READ_FILE_TOOL_DESCRIPTION = r"""Reads a file from the filesystem. You can access any file directly by using this tool.
Assume this tool is able to read all files on the machine. If the User provides a path to a file assume that path is valid. It is okay to read a file that does not exist; an error will be returned.

Usage:
- The file_path parameter must be an absolute Windows path
  - Windows native format: `C:/Users/name/file.txt` (forward slashes)
...
EDIT_FILE_TOOL_DESCRIPTION = r"""Performs exact string replacements in files.
...
WRITE_FILE_TOOL_DESCRIPTION = r"""Writes to a new file in the filesystem.
...
- Prefer to editing existing files over creating new ones when possible."""


GLOB_TOOL_DESCRIPTION = r"""Find files matching a glob pattern.

Usage:
- The glob tool finds files by matching patterns with wildcards
- Supports standard glob patterns: `*` (any characters), `**` (any directories), `?` (single character)
- Patterns must use Windows path format
- Returns a list of absolute Windows file paths that match the pattern

Examples:
- glob(pattern="*.txt", path="D:/Projects")
- glob(pattern="**/*.py") — searches current directory recursively"""

GREP_TOOL_DESCRIPTION = r"""Search for a pattern in files.
...
EXECUTE_TOOL_DESCRIPTION = r"""Executes a given command in the sandbox environment with proper handling and security measures.
...
2. Command Execution:
   ...
   - Examples of proper quoting:
     - cd "C:/Users/name/My Documents" (correct)
     - cd C:/Users/name/My Documents (incorrect - will fail)
     - python "C:/path/with spaces/script.py" (correct)
     - python C:/path/with spaces/script.py (incorrect - will fail)
...
Examples:
  Good examples:
    - execute(command="pytest C:/foo/bar/tests")
    - execute(command="python C:/path/to/script.py")
    - execute(command="npm install && npm test")

  Bad examples (avoid these):
    - execute(command="cd C:/foo/bar && pytest tests")  # Use absolute path instead
...
FILESYSTEM_SYSTEM_PROMPT = r""" ##Filesystem Tools `ls`, `read_file`, `write_file`, `edit_file`, `glob`, `grep`
...
def _ls_tool_generator(
    ...
    def ls(runtime: ToolRuntime[None, FilesystemState], path: str) -> str:
        ...
        try:
            validated_path, was_virtual = _validate_path(path)
        except ValueError as e:
            return str(e)
        
        # If virtual path was used, return warning immediately
        if was_virtual:
            return "Error: Git Bash virtual path format detected. Please use Windows native path format (e.g., C:/Users/name/file.txt) instead of /c/Users/name/file.txt format."
...
def _read_file_tool_generator(
    ...
    def read_file(
        ...
        resolved_backend = _get_backend(backend, runtime)
        try:
            file_path, was_virtual = _validate_path(file_path)
        except ValueError as e:
            return str(e)
...
def _glob_tool_generator(
    ...
    def glob(pattern: str, runtime: ToolRuntime[None, FilesystemState], path: str = ".") -> str:
        resolved_backend = _get_backend(backend, runtime)
        if _is_absolute_glob_pattern(pattern):
            return (
                "Error: Absolute file paths are not valid glob patterns. "
                "Use a relative glob pattern such as uploads/*.pdf or provide a Windows path in the path parameter."
            )
        if path != ".":
            try:
                path, was_virtual = _validate_path(path)
            except ValueError as e:
                return str(e)
            if was_virtual:
                return "Error: Git Bash virtual path format detected. Please use Windows native path format (e.g., C:/Users/name/file.txt) instead of /c/Users/name/file.txt format."
...
class FilesystemMiddleware(AgentMiddleware):
    ...
    def _process_large_message(
        ...
        # 单独适配了windows的临时目录，超大输出就写道临时目录
        import tempfile
        temp_dir = tempfile.gettempdir()
        base_path = os.path.abspath(os.path.join(temp_dir, "large_tool_results"))
        os.makedirs(base_path, exist_ok=True)
        file_path = os.path.join(base_path, sanitized_id).replace("\\", "/")
