...
def _normalize_glob_pattern(pattern: str, search_path: Path) -> str | None:
    """Normalize absolute glob patterns into search_path-relative patterns.

    Returns None when the absolute pattern points outside search_path.
    """
    normalized = pattern.replace("\\", "/")
    if not normalized:
        return normalized

    absolute_windows = re.match(r"^[A-Za-z]:/", normalized)
    absolute_posix = normalized.startswith("/")
    if not absolute_windows and not absolute_posix:
        return normalized

    candidate = Path(normalized)
    try:
        relative = candidate.relative_to(search_path)
    except ValueError:
        return None

    relative_pattern = str(relative).replace("\\", "/")
    return relative_pattern or "."
...
class FilesystemBackend(BackendProtocol):
    ...
    def _read_text_with_fallback(self, filepath: Path, secure: bool = False, return_encoding: bool = False) -> str | tuple[str, str] | None:
        """Read file content with multiple encoding fallbacks.
        
        Args:
            filepath: Path to the file to read
            secure: If True, use O_NOFOLLOW to prevent symlink traversal
            return_encoding: If True, return (content, encoding) tuple
            
        Returns:
            File content as string, or (content, encoding) tuple if return_encoding=True,
        ...
        if secure:
            ...
            # Normal reading without O_NOFOLLOW (for grep/search operations)
            for encoding in encodings:
                try:
    ...
    def read(
        ...
        try:
            # Read securely with encoding fallback
            content = self._read_text_with_fallback(resolved_path, secure=True, return_encoding=False)
            if content is None:
    ...
    def edit(
        ...
        try:
            # Read securely with encoding detection
            read_result = self._read_text_with_fallback(resolved_path, secure=True, return_encoding=True)
            if read_result is None:
                return EditResult(error=f"Error: Unable to read file '{file_path}'")
            
            content, original_encoding = read_result
            ...
            # Write securely with original encoding
            flags = os.O_WRONLY | os.O_TRUNC
            if hasattr(os, "O_NOFOLLOW"):
                flags |= os.O_NOFOLLOW
            fd = os.open(resolved_path, flags)
            with os.fdopen(fd, "w", encoding=original_encoding) as f:
                f.write(new_content)
    ...
    def _python_search(self, pattern: str, base_full: Path, include_glob: str | None) -> dict[str, list[tuple[int, str]]]:
        ...
        for fp in root.rglob("*"):
            ...
            try:
                ...
                continue
            content = self._read_text_with_fallback(fp, secure=False, return_encoding=False)
            if content is None:
                continue
    ...
    def glob_info(self, pattern: str, path: str = ".") -> list[FileInfo]:
        ...
        normalized_pattern = _normalize_glob_pattern(pattern, search_path)
        ...
        try:
            ...
            for matched_path in search_path.rglob(normalized_pattern):
                try:
