...
class EnterpriseKnowledgeError(RuntimeError):
    """企业知识引擎调用失败时抛出的统一业务异常。"""
    pass


# 模块级缓存：
# 当前运行时只从数据库 kb_config 表同步进来，不再在这里主动回退到 config.py。
_R2R_BASE_URL: str | None = None


def set_r2r_base_url(url: str | None) -> None:
    """更新模块级 R2R Base URL 缓存。"""
...
def check_r2r_reachable() -> bool:
    ...
    判定标准比较宽：
    - 2xx / 3xx 视为可达
    - 401 / 403 也视为可达，因为这通常说明端点存在，只是需要认证
    - 404 继续尝试下一个健康检查路径
    """
...
def _r2r_request(
    ...
    """向 R2R 发起统一请求，并把常见失败翻译成 ``EnterpriseKnowledgeError``。

    这里统一做三件事：
    - 用 ``_R2R_BASE_URL`` 组装目标地址
    - 用 ``primary_key`` 生成 ``Authorization: Bearer dev:{primary_key}``
    - 把超时、连不上、HTTP 错误转换成平台自己的异常
    """
    if not _R2R_BASE_URL:
...
def search_enterprise(
    ...
    当前请求体的组装规则：
    - ``collection_ids`` 若提供，会在这里转回 int 列表
    - ``graph_limits`` 只有在 ``use_graph_search=True`` 时才写入 payload
    """
    payload: dict = {
        "query": query,
...
def rag_enterprise(
    ...
    与 ``search_enterprise()`` 共享大部分检索参数，
    额外固定塞入一份 ``rag_generation_config``。
    """
    payload: dict = {
        "query": query,
        "collection_ids": [int(cid) for cid in collection_ids] if collection_ids else None,
...
def upload_enterprise_document(
    ...
    """上传文档到某个企业知识 collection。

    当前固定行为：
    - ``metadata`` 传空对象
    - ``collection_ids`` 只包含当前 collection_id
    - ``ingestion_mode`` 固定为 ``fast``
    """
