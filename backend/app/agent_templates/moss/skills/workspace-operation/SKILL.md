...
description: 当 MOSS 需要管理工作空间及其子资源时使用，包括工作空间列表、创建、详情、更新、删除，以及工作空间绑定的知识库管理。
---

# MOSS 工作空间操作

只处理 CamphorEOS 工作空间及其子资源。
...
### 创建工作空间
- 触发条件：用户要新建工作空间，且名称、目标与工作目录已经明确。
- 接口路由：`POST /api/workspaces`
- 必填参数：请求体 `name`、`working_dir`
- 返回关键字段：`id`、`name`、`goal`、`working_dir`
- 硬性约束：`name` 必填；`working_dir` 必须是绝对路径；名称不能重复。
...
- 硬性约束：`workspace_id` 必须存在；删除时会同时清理该空间业务数据、关联目录、运行时目录与会话。
