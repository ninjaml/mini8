...
管理 MOSS 的 cron job，包括任务生命周期管理与执行历史回溯。
...
- 用户想查看有哪些定时任务 → 动作一：列出任务（L41 - L86）
- 用户想新增一个定时任务 → 动作二：创建任务（L87 - L143）
- 用户想查看某个任务的完整配置或最近状态 → 动作三：获取任务详情（L144 - L187）
- 用户想修改任务（名称、调度周期、prompt、启用状态）→ 动作四：修改任务（L188 - L243）
- 用户想删除一个定时任务 → 动作五：删除任务（L244 - L270）
- 用户想立即执行一次任务，不等定时触发 → 动作六：手动触发（L271 - L305）
- 用户想暂停或恢复某个任务的自动触发 → 动作七：启用/禁用切换（L306 - L354）
- 用户想查看所有任务的最近执行摘要 → 动作八：历史概览（L355 - L397）
- 用户想查看某个任务的具体执行记录 → 动作九：历史详情（L398 - L472）
...
`GET /api/runtime/cron/jobs?agent_name=moss`

说明：这个接口也支持 `workspace_id` 过滤，但 MOSS 自身仍按 `agent_name=moss` 查询。

#### 请求参数

查询参数：

- `agent_name`：可选，固定传 `moss`
...
[
  {
    ...
    "schedule": "0 9 * * *",
    "prompt": "生成今日晨报摘要",
    "thread_id": "cron-moss-daily-report-xxxxx",
    "working_dir": "/path/to/moss",
    "enabled": true,
...
`GET /api/runtime/cron/history?kind=moss`
