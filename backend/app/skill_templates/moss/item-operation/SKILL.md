---
name: CamphorEOS-moss-item-operation
description: 当 MOSS 需要在某个工作空间中查看、创建、修改、绑定、审核或删除事项与成果历史时使用。
---
...
### 查看事项列表
- 触发条件：用户想看某个工作空间有哪些事项，或为修改、绑定成员、查看成果先定位事项。
...
### 创建事项
- 触发条件：目标工作空间、事项名称和核心要求已明确，且用户确认创建。
...
对 MOSS 来说，`workspace_id` 不默认来自上下文。

获取方式：

1. 先调用 `GET /api/workspaces`
2. 根据用户给出的空间名称、描述或候选信息定位目标空间
3. 如果出现多个候选，先确认
...
1. 先确定 `workspace_id`
2. 调用 `GET /api/workspaces/{workspace_id}/items`
3. 根据事项名称、状态或候选信息定位目标事项
4. 如果出现多个候选，先确认
...
1. 先确定 `workspace_id`
2. 调用 `GET /api/workspaces/{workspace_id}/agents`
3. 根据成员名称定位目标成员
4. 如果出现多个候选，先确认
...
[
  {
    ...
    "work_space_id": 1,
    "name": "竞品内容策略拆解",
    "description": "拆解近期爆款内容的选题与分发策略",
    "work_requirement": "输出结构化结论，标注关键样例与判断依据",
    "delivery_requirement": "提交 Markdown 或文档成果，便于后续审核",
...
{
  "name": "竞品内容结构拆解",
  "description": "聚焦近期增长样例，更新分析范围",
  "work_requirement": "增加样例拆解与结论对照",
  "delivery_requirement": "输出 Markdown 结论稿",
  "need_superagent_review": true,
  "need_superone_review": false,
  "allow_auto_complete": false
}
...
[
  {
    ...
    "work_item_id": 3,
    "title": "竞品选题趋势图谱 v1",
    "summary": "已提交核心选题结构与增长机制判断，等待 PM 审核。",
    "submitted_by_name": "Athena 数据专员",
    "status": "reviewing",
    "file_count": 2,
    "files": [
      {
        "name": "trend-map-v1.md",
        "size": 2048
      },
      {
...
```json
{
  ...
  "file_count": 0,
  "files": [],
  "preview_text": "已提交核心选题结构与增长机制判断，等待 PM 审核。"
}
```
...
```json
{
  ...
  "superagent_review_note": "结论完整，可以进入成果库。"
}
```
