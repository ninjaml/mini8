...
| 动作 | 方法与路径 | 说明 |
|---|---|---|
| 查看知识库列表 | `GET /api/workspaces/{workspace_id}/knowledge` | 返回指定工作空间已绑定的知识库 |
| 创建知识库 | `POST /api/workspaces/{workspace_id}/knowledge` | 创建 Obsidian 知识库绑定 |
| 修改知识库名称 | `PATCH /api/knowledge/{knowledge_id}` | 只修改知识库显示名称 |
| 解除知识库绑定 | `DELETE /api/workspaces/{workspace_id}/knowledge/{knowledge_id}` | 删除该工作空间下的知识库绑定 |
...
MOSS 没有默认绑定工作空间。需要先定位目标工作空间：

1. 调用 `GET /api/workspaces` 获取候选工作空间。
2. 根据用户给出的名称、描述或上下文定位目标。
3. 如果存在多个候选，先让用户确认。
4. 目标明确后，才能使用对应 `workspace_id`。

### `knowledge_id`

1. 已知 `workspace_id` 后，调用 `GET /api/workspaces/{workspace_id}/knowledge`。
...
```json
{
  ...
  "name": "我的 Obsidian Vault",
  "type": "obsidian",
  "knowledge_json": "{\"port\":27123,\"api_key\":\"***\",\"vault_name\":\"我的 Obsidian Vault\",\"omnisearch_port\":51361}"
}
```
...
- 成功返回 `204 No Content`。
- `knowledge_id` 必须属于该 `workspace_id`。
