---
name: CamphorEOS-superagent-cron-scheduler
description: 当 SuperAgent 需要管理当前工作空间的定时任务（创建、查询、修改、删除、手动触发、查看执行历史）时使用。
---

# SuperAgent 定时任务调度器

管理当前工作空间的 cron job，包括任务生命周期管理与执行历史回溯。
...
- 用户想查看当前工作空间有哪些定时任务 → 动作一：列出任务（L45 - L90）
- 用户想为当前工作空间新增一个定时任务 → 动作二：创建任务（L91 - L147）
...
对 SuperAgent 来说，`workspace_id` 默认来自当前绑定工作空间上下文。创建或查询 cron 任务时，`target_id`（即 `workspace_id`）直接使用该值。
...
`GET /api/runtime/cron/jobs?agent_name=workspace-{workspace_id}-superagent`

#### 请求参数

查询参数：

- `agent_name`：可选，如 `workspace-1-superagent`
...
[
  {
    ...
    "target_id": 1,
    "agent_name": "workspace-1-superagent",
    "name": "每日项目汇报",
    "schedule": "0 9 * * *",
    "prompt": "生成今日项目进展汇报",
    "thread_id": "cron-workspace_superagent-1-daily-report-xxxxx",
    "working_dir": "/path/to/workspace/1",
    "enabled": true,
...
- `kind` 固定为 `"workspace_superagent"`
- `target_id` 必须使用当前 `workspace_id`（SuperAgent 没有独立 id，其身份由 `workspace_id` 唯一确定）
...
- `kind` 和 `target_id`（即 `workspace_id`）不可变更
- 修改 `schedule` 会自动重新调度
...
`GET /api/runtime/cron/history?kind=workspace_superagent&target_id={workspace_id}`

#### 请求参数

查询参数：

- `kind`：固定 `"workspace_superagent"`
- `target_id`：当前 `workspace_id`（SuperAgent 的身份标识字段）
